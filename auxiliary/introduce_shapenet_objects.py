...
def add_shapenet_objects_to_kitti_points(spn_points, kitti_points):
    ...
    temp_vars.introduced_anomaly_indices = np.zeros(kitti_points.shape[0], dtype=bool)
    r, lat, lon = ac.cartesian_to_spherical(spn_points[:, 0], spn_points[:, 1], spn_points[:, 2])
    ...
    if overflow:
        ...
        overlap_indices = (overlap_indices_right_half | overlap_indices_left_half) & overlap_indices_lat

        temp_vars.overlap_indices = overlap_indices
    ...
    for i in range(overlap_indices.shape[0]):
        ...
        temp_vars.introduced_anomaly_indices[i] = True
