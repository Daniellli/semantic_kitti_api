...
class LaserScan:
    ...
    def open_scan(self, filename):
        ...
        if False:
            ...
            path = '../demo_shapenet/03636649/d8d129b1a07b23b7a738de48265832af/pointcloud.npz'
            spn_points = load_shapenet_object(path)

            # Rotate shapenet object to upright orientation
            spn_points = rotate_shapenet_object(spn_points, dx=90, dy=0, dz=30)

            # Move shapenet object
            spn_points = move_shapenet_object(spn_points, dx=5)
            spn_points = move_shapenet_object_in_sphere(spn_points, dlon=-60 - 50 - 90 - 45 - 30)

            # Resize shapenet object
            spn_points = resize_shapenet_object(spn_points, resize_factor=1)

            # Put shapenet object on the ground
            spn_points = put_shapenet_object_on_the_ground(spn_points, points)

            # Add shapenet points into kitti points
            points = add_shapenet_objects_to_kitti_points(spn_points, points)
...
class SemLaserScan(LaserScan):
    ...
    def set_label(self, label):
        ...
        # TODO: temp: replace labels. Change 99 to something else?
        if temp_vars.introduced_anomaly_indices is not None:
            label[temp_vars.introduced_anomaly_indices] = 100
