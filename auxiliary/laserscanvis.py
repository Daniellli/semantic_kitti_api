...
class LaserScanVis:
  ...
  def update_scan(self):
    ...
    # Save point cloud with added shapnet object in XYZRGB format
    if False:
      mapped_rgb_color = self.scan.sem_label_color
      combined_points = self.scan.points
      points_with_rgb = np.hstack([combined_points, mapped_rgb_color * 255])
      np.savetxt('combined_points_with_rgb_color.txt', points_with_rgb)
      np.savez('combined_points_with_rgb_color.npz', points_with_rgb)
